...
public class DiscountOptimizer {
	...
	private static Set<Integer> newPurchase(int setLimit, 
		...
		while(newPurchaseSet.size() != setLimit) {
			...
			Integer nextUnit = nextUnitToPurchase(newPurchaseSet, availUnits);
	 ...
	 */
	private static Integer nextUnitToPurchase(Set<Integer> purchasedSet, 
			Map<Integer, Integer> availUnits) {
	...
	private void initalization() {
		...
		IntStream.range(1, DISCOUNTS.length+1).forEach(x -> {
			...
			valueIncreases.put(x, prizeMap.get(x).subtract(prizeMap.getOrDefault(x-1, 
					new BigDecimal("0.00"))));
		});
